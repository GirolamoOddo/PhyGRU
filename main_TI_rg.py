...
class PhyGRUCell(nn.Module):
    def __init__(self, state_dim, input_dim, physics_law, latent_dim=0):
        ...
        if latent_dim > 0:
            ...
            # GRU-style reset gate applied to latent terms only:
    ...
    def forward(self, state, u):
        ...
        if self.latent_dim > 0:
            ...
            # compute reset gate for latent terms (GRU-style)
            r = self.r_gate(torch.cat([state, u], dim=1))
            # apply reset gate to previous latent before computing latent dynamics
            gated_latent = r * latent
            # construct a "gated" total_state vector: [physical_state, gated_latent]
