...
def generate_data_1(u_fn):
    ...
    for t in range(T):
        u = u_fn(t)
        xdd = (u - (0.5 + 0.00009*t) * xd - (0.2 + 0.0000001*t**2) * x)
        xd += dt * xdd
        x  += dt * xd
        xs.append([x])
        us.append([u])
...
def generate_data_2(u_fn):
    ...
    for t in range(T):
        u = u_fn(t)
        xdd = (u - (0.5 + 0.00009*t) * xd - (0.2 + 0.0000001*t**2) * x + (1 + 0.00009*t) * math.tanh(x*xd))
        xd += dt * xdd
        x  += dt * xd
        xs.append([x])
        us.append([u])
...
def generate_data_3(u_fn):
    ...
    for t in range(T):
        u = u_fn(t)
        xddd = u - (3 - 0.0002*t) * xdd - (2 + 0.0009*t)*math.tanh(xd) - (0.1 + 0.0000003*t**2) * math.tanh(x) 
        xdd += dt * xddd
        xd  += dt * xdd
        x   += dt * xd
        xs.append([x])
